...
_RAW_LENDERS = [
    ...
    ("Central Murray Bank", "https://secure.centralmurray.bank/openbanking/cds-au/v1"),
    ...
    ("Darling Downs Bank", "https://openbanking.ddbank.com.au/OpenBanking/cds-au/v1"),
...
def get_cdr_bank_registry() -> List[Dict[str, object]]:
    records: List[Dict[str, object]] = []
    for name, base_url in _RAW_LENDERS:
        lower_name = name.lower()
        category = "major_bank" if any(token in lower_name for token in ("anz", "commbank", "westpac", "national australia bank", "nab")) else "lender"
        records.append(
            {
                "id": f"cdr-{_slug(name)}",
                "name": name,
                "brand": name,
                "base_url": base_url,
                "api_family": "cdr_banking_public",
                "product_path": "/banking/products",
                "category": category,
                "supports_public_products": True,
                "supports_consumer_data": False,
                "active": True,
                "notes": "Seeded from operator-supplied CDR banking endpoint registry.",
                "metadata_json": {
                    "source": "operator_seed",
                    "product_endpoint": f"{base_url.rstrip('/')}/banking/products",
                    "detail_endpoint_template": f"{base_url.rstrip('/')}/banking/products/{{productId}}",
                },
            }
        )
    return records
